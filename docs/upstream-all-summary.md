...
- #105 [open, mergeable=clean, mirrored=yes] fix: security improvements and error handling fixes (`fix/security-improvements` -> `main`)
  - local coverage [partial]: Safe subset landed locally for JSON error sanitization and parser logging, but the behavior-changing `DEBUG`, `SECRET_KEY`, and stricter default CORS pieces remain deferred for compatibility review.
