...
- State filter: `all`
- Captured: `2026-03-11T12:23:40.229223+00:00`
...
- #117 [open] ### Feature Request: English Language Support (enhancement)
  - local coverage [covered]: The remaining Step 3 timeline labels, Step 5 platform/status labels, and locale-aware interaction timestamps now flow through the shared i18n dictionaries, so the English mode no longer falls back to hardcoded workflow strings in those surfaces.
...
- #105 [open, mergeable=clean, mirrored=yes] fix: security improvements and error handling fixes (`fix/security-improvements` -> `main`)
  - local coverage [partial]: Local coverage now includes the error-handling hardening, env-driven CORS controls, `DEBUG=False` by default, and generated fallback `SECRET_KEY` behavior; only the stricter default CORS-origin policy remains intentionally deferred for backward compatibility.
