...
- `#82` Upstream's dependency-only CVE patch is still not directly cherry-pickable: it only edits `backend/requirements.txt`, while this repo now isolates the vulnerable OASIS runtime behind an optional install path. The remaining follow-up is to replace or vendor that optional runtime so Step 3 / Step 5 no longer depend on `camel-oasis==0.2.5`.
